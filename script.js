...
(function() {
  ...
  window.APP = APP = new ((function() {
    ...
    _Class.prototype.initTriangle = function() {
      var col, datum, facesData, fiddle, hexagon, i, offsetX, offsetY, row, size, triangleFacesData, triangleVertexData, verticiesData, _i, _j, _k, _l, _len, _len1, _len2, _m, _ref;
      size = 0.2;
      fiddle = {
        x: 1 / 10,
        y: 1 / 10 * TAN30 / 2
      ...
      this.hexagons = [];
      for (row = _i = -5; _i <= 5; row = ++_i) {
        for (col = _j = -5; _j <= 5; col = ++_j) {
          if (Math.abs(row % 2) === 1) {
            this.hexagons.push(new Hexagon(col * (offsetX + size), row * offsetY, size));
          } else {
            this.hexagons.push(new Hexagon((3 + fiddle.x) * size / 2 + col * (offsetX + size), row * offsetY, size));
          }
        }
      }
      triangleVertexData = [];
